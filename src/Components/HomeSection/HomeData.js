import room1 from "../../assets/images/Room-1.jpg";
import room2 from "../../assets/images/Room-2.jpg";
import room3 from "../../assets/images/Room-3.jpg";
import room4 from "../../assets/images/Room4.jpg";
import room5 from "../../assets/images/Room5.webp";
import Room6 from "../../assets/images/Room6.jpg";
import Hotel12 from "../../assets/images/Hotel14.jpg";
import Hotel14 from "../../assets/images/Hotel16.webp";
import Hotel16 from "../../assets/images/Hotel18.jpeg";
import Hotel13 from "../../assets/images/Hotel15.webp";
import Hotel15 from "../../assets/images/Hotel17.jpg";
import Hotel6 from "../../assets/images/Hotel-7.jpg";
import Hotel17 from "../../assets/images/Hotel9.jpg";
import Hotel7 from "../../assets/images/Hotel-8.jpg";
import BEACH from "../../assets/images/Hotel-7.jpg";
import Plane from "../../assets/images/plane1.jpg";
import Part1 from "../../assets/images/part1.png";
import Part2 from "../../assets/images/part2.webp";
import Part3 from "../../assets/images/part3.webp";
import Bedroom from "../../assets/images/bedroom1.jpg";
import Wifi from "../../assets/images/wifi.png";
import Aircon from "../../assets/images/air-conditioner.png";
import meeting from "../../assets/images/meeting.png";
import shuttle from "../../assets/images/shuttle.png";
import parking from "../../assets/images/parking.png";
import restaurants from "../../assets/images/spaghetti.gif";
import disabled from "../../assets/images/disabled-person.png";
import smartphone from "../../assets/images/smartphone_thumbnail.png";
import iphone from "../../assets/images/iphone_thumbnail.png";
import { FaApple, FaGooglePlay } from "react-icons/fa";

// ======= Hero Page Data =====
export const HeroSlide = [
  {
    id: 1,
    cover: room1,
  },
  {
    id: 2,
    cover: room2,
  },
  {
    id: 3,
    cover: room3,
  },
  {
    id: 4,
    cover: room4,
  },
  {
    id: 5,
    cover: room5,
  },
  {
    id: 6,
    cover: Room6,
  },
];

//   ===== Most Popular Hotels Data =====
export const Mostpopularhotel = [
  {
    id: 1,
    cover: room2,
    name: "HighWay Hotel",
    price: 300.0,
    discount: 8.5,
    rating: 4,
  },
  {
    id: 2,
    cover: room3,
    name: "Accra",
    price: 450.0,
    discount: 8,
    rating: 4.5,
  },
  {
    id: 3,
    cover: room4,
    name: "Lagos",
    price: 400.0,
    discount: 10,
    rating: 3.5,
  },
  {
    id: 4,
    cover: room5,
    name: "Cape Coast",
    price: 300.0,
    discount: 2,
    rating: 4.8,
  },
  {
    id: 5,
    cover: room1,
    name: "Turkey",
    price: 480.0,
    discount: 8,
    rating: 3,
  },
  {
    id: 6,
    cover: Hotel12,
    name: "California",
    price: 290.0,
    discount: 3,
    rating: 5,
  },
  {
    id: 7,
    cover: Hotel14,
    name: "Dubai",
    price: 400.0,
    discount: 4,
    rating: 4.5,
  },
  {
    id: 8,
    cover: Hotel16,
    name: "Dubai",
    price: 500.0,
    discount: 9,
    rating: 3.5,
  },
  {
    id: 9,
    cover: Hotel17,
    name: "Kumerica",
    price: 600.0,
    discount: 8,
    rating: 4,
  },
  {
    id: 10,
    cover: Hotel15,
    name: "Kumerica",
    price: 350.0,
    discount: 3.5,
    rating: 4.5,
  },
  {
    id: 11,
    cover: Hotel13,
    name: "Cape Coast",
    price: 450.0,
    discount: 5,
    rating: 4.5,
  },
  {
    id: 12,
    cover: Hotel6,
    name: "Kumasi",
    price: 400.0,
    discount: 3.5,
    rating: 4.5,
  },
];

//   ====== Service Data ======

export const ServiceProvide = [
  {
    id: 1,
    cover: Wifi,
    name: "Free Wifi",
  },
  {
    id: 2,
    cover: Aircon,
    name: "24/7 Air Conditioned",
  },
  {
    id: 3,
    cover: restaurants,
    name: "Restaurant",
  },
  {
    id: 4,
    cover: parking,
    name: "Secured Parking",
  },
  {
    id: 5,
    cover: shuttle,
    name: "Free Shuttle",
  },
  {
    id: 6,
    cover: meeting,
    name: "Meeting Halls",
  },
  {
    id: 7,
    cover: disabled,
    name: "Disabled Person",
  },
];

// ======= Partnership Data =======
export const OurPartnership = [
  {
    id: 1,
    cover: Plane,
    company: "Company Name",
    desc: "Lorem ipsum dolor sit, amet consectetur adipisicing elit. Dignissimos dolorum repudiandae?",
  },
  {
    id: 2,
    cover: Part1,
    company: "Company Name",
    desc: "Lorem ipsum dolor sit, amet consectetur adipisicing elit. Dignissimos dolorum repudiandae?",
  },
  {
    id: 3,
    cover: Plane,
    company: "Company Name",
    desc: "Lorem ipsum dolor sit, amet consectetur adipisicing elit. Dignissimos dolorum repudiandae?",
  },
  {
    id: 4,
    cover: Part3,
    company: "Company Name",
    desc: "Lorem ipsum dolor sit, amet consectetur adipisicing elit. Dignissimos dolorum repudiandae?",
  },
  {
    id: 6,
    cover: Part2,
    company: "Company Name",
    desc: "Lorem ipsum dolor sit, amet consectetur adipisicing elit. Dignissimos dolorum repudiandae?",
  },
  {
    id: 7,
    cover: Plane,
    company: "Company Name",
    desc: "Lorem ipsum dolor sit, amet consectetur adipisicing elit. Dignissimos dolorum repudiandae?",
  },
];

//   ===== Discover Data ======
export const DisCover = [
  {
    id: 1,
    cover: Bedroom,
    location: "REST ASSURED LAGOS - NIGERIA",
  },
  {
    id: 3,
    cover: Hotel16,
    location: "REST ASSURED ACCRA - GHANA",
  },
  {
    id: 4,
    cover: Hotel7,
    location: "REST ASSURED CALIFORNIA - USA",
  },
  {
    id: 5,
    cover: BEACH,
    location: "REST ASSURED MIAMI - USA",
  },
];

export const DownloadApp = [
  {
    title: "Download our app",
    SubTile1: "Get our Template App",
    SubTile2: "For Your Mobile",
    procedure: [
      { display: "Find nearby hotel in your network with template" },
      { display: "Get paperless confirmation" },
      { display: "Make changes whenever, wherever" },
      { display: "24/7 customer service in more than 40 languages" },
      { display: "No booking or credit card fees" },
      { display: "Add your own reviews and photos" },
    ],
    mobile: [{ phone: iphone }, { phone: smartphone }],
    get: [
      { icon: <FaApple />, name: "iOS" },
      { icon: <FaGooglePlay />, name: "Andriod" },
    ],
  },
];
